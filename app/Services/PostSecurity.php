...
/**
 * Community post / comment feature.
 * Algorithm: from-scratch ECC NIST P-256 (same ECIES construction as items, implemented in this file as PostEcc).
 */
class PostSecurity
{
    ...
    public function encryptPost(array $fields): array
    {
        $pub = $this->keys->eccPublic();
        $title = $this->ecc->encrypt($fields['title'], $pub);
        $content = $this->ecc->encrypt($fields['content'], $pub);

        return [
            'title' => $title,
            'content' => $content,
            'mac' => $this->mac->generate($this->mac->join([$title, $content])),
        ];
    }

    public function hydratePost(Post $post): ?Post
    {
        try {
            $title = (string) ($post->getRawOriginal('title') ?? $post->title);
            $content = (string) ($post->getRawOriginal('content') ?? $post->content);
            $mac = (string) ($post->getRawOriginal('mac') ?? $post->mac ?? '');

            if ($mac === '' || !$this->mac->verify($this->mac->join([$title, $content]), $mac)) {
                throw new RuntimeException('Post integrity check failed.');
            }

            $post->setAttribute('title', $this->ecc->decrypt($title, $this->keys->resolveEccPrivateFromCipher($title)));
            $post->setAttribute('content', $this->ecc->decrypt($content, $this->keys->resolveEccPrivateFromCipher($content)));
            return $post;
        } catch (\Throwable) {
            return null;
        }
    ...
    public function encryptComment(string $content): array
    {
        $cipher = $this->ecc->encrypt($content, $this->keys->eccPublic());
        return [
            'content' => $cipher,
            'mac' => $this->mac->generate($cipher),
        ];
    }

    public function hydrateComment(Comment $comment): ?Comment
    {
        try {
            $content = (string) ($comment->getRawOriginal('content') ?? $comment->content);
            $mac = (string) ($comment->getRawOriginal('mac') ?? $comment->mac ?? '');
            if ($mac === '' || !$this->mac->verify($content, $mac)) {
                throw new RuntimeException('Comment integrity check failed.');
            }
            $comment->setAttribute('content', $this->ecc->decrypt($content, $this->keys->resolveEccPrivateFromCipher($content)));
            return $comment;
        } catch (\Throwable) {
            return null;
        }
